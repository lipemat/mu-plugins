...
class Cache_Replace_Test extends Object_Cache_TestCase {
	public function test_replace_value_with_another_value() : void {
		$key = microtime();
		...
		$this->assertCachePropertyAndExternal( $key, $value2 );
	}


	public function test_replace_value_when_key_is_not_set() : void {
		$key = microtime();
	...
	public function test_replace_with_expiration_of_30_days() {
		...
		$value     = 'merica';
		$group     = 'july';
		$this->object_cache->key( $key, $group );
		...
		$this->assertCachePropertyAndExternal( $key, $value2, $group );
	}


	public function test_replace_with_expiration_longer_than_30_days() : void {
		$key = 'usa';
		$value = 'merica';
		$group = 'july';
